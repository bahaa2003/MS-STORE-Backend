...
const requireCompatOrderingAvailable = async (_req, res, next) => {
    try {
        ...
        if (setting?.value === true) return res.status(503).json({ status: 'ERROR', code: ERROR_CODES.MAINTENANCE, message: 'Site is under maintenance' });
        return next();
    } catch (_) { return res.status(500).json({ status: 'ERROR', code: 500, message: 'Unknown internal error' }); }
};
